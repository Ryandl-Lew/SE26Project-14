...
class FileControllerIntegrationTest {
    ...
    void setUp() {
        ...
        // 插入 mock-user-id 对应用户，满足 fk_attachments_uploader 外键约束
        jdbcTemplate.update(
                "MERGE INTO users (id, username, password_hash, name, email, avatar_text, status, created_at, updated_at) " +
                "KEY (id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
                "mock-user-id",
                "mockuser",
                "$2a$10$dummy",
    ...
    void testUploadProjectFileAndDownload() throws Exception {
        ...
        assertThat(dbRecord.get().getUploadedBy()).as("uploadedBy 为 mock-user-id")
                .isEqualTo("mock-user-id");
