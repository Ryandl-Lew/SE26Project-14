/**
 * 页面组件出口
 */
export { default as DashboardPage } from './DashboardPage'
export { default as ProjectsPage } from './ProjectsPage'
export { default as ProjectDetailPage } from './ProjectDetailPage'
export { default as RecordsPage } from './RecordsPage'
export { default as RecordCreatePage } from './RecordCreatePage'
export { default as RecordEditorPage } from './RecordEditorPage'
export { default as RecordDetailPage } from './RecordDetailPage'
export { default as TemplatesPage } from './TemplatesPage'
export { default as SearchPage } from './SearchPage'
export { default as ProfilePage } from './ProfilePage'
export { default as AiAssistantPage } from './AiAssistantPage'
export { default as NotFoundPage } from './NotFoundPage'
export { default as LoginPage } from './LoginPage'
export { default as RegisterPage } from './RegisterPage'
